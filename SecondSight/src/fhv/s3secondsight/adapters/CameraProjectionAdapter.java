package fhv.s3secondsight.adapters;
...
public class CameraProjectionAdapter {
	...
	public MatOfDouble getProjectionCV() {
		if (mProjectionDirtyCV) {
			...
			double diagonalPx = Math.sqrt((Math.pow(mWidthPx,  2.0) + Math.pow(mHeightPx,  2.0)));
			double diagonalFOV = Math.sqrt((Math.pow(mFOVX, 2.0) + Math.pow(mFOVY, 2.0)));
			double focalLengthPx = diagonalPx / (2.0 * Math.tan(0.5 * diagonalFOV));
